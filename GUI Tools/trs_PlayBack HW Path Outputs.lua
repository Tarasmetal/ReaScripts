-- @description Playback Routing Manager
...
local function myWindow()
  ...
  if reaper.ImGui_BeginPopupModal(ctx, 'Save Preset Dialog', true, reaper.ImGui_WindowFlags_AlwaysAutoResize()) then
      reaper.ImGui_Text(ctx, 'Enter preset name:')
  ...
  reaper.ImGui_TextColored(ctx, 0xFFFFFFFF, 'Routing Settings:')
  reaper.ImGui_Separator(ctx)
  ...
  for i, settings in ipairs(tr_settings) do
    ...
    local changed_type, current_type_idx = reaper.ImGui_Combo(ctx, '##type_' .. i, (settings.type == 's' and 0 or 1), 'stereo\0mono\0')
