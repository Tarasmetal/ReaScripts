...
local folder_buttons = {} -- Кнопки для папок первого уровня
local last_project_change_count = -1 -- Для отслеживания изменений в проекте
...
-- Функция для получения папок первого уровня
function GetFirstLevelFolders()
    ...
    for i = 0, track_count - 1 do
        ...
        -- Проверяем, является ли трек папкой первого уровня (depth == 0)
        if depth == 0 and folder_depth > 0 then
            local _, name = reaper.GetSetMediaTrackInfo_String(track, "P_NAME", "", false)
            if name == "" then name = "Folder " .. (i + 1) end
            table.insert(folders, {track = track, name = name, index = i})
        end
    end
    return folders
end

-- Функция для управления видимостью папки и её содержимого
function SetFolderVisibility(folder_track, folder_index, mixer_mode)
    local track_count = reaper.CountTracks(0)
    
    reaper.Undo_BeginBlock()
    
    -- Если Shift не нажат, скрываем другие треки в зависимости от режима
    if not is_shift_pressed then
        for i = 0, track_count - 1 do
            local t = reaper.GetTrack(0, i)
            if mixer_mode == 't' then
                -- TCP режим: скрываем в TCP, оставляем в микшере
                reaper.SetMediaTrackInfo_Value(t, "B_SHOWINTCP", 0)
                reaper.SetMediaTrackInfo_Value(t, "B_SHOWINMIXER", 1)
            elseif mixer_mode == 'm' then
                -- Mixer режим: оставляем в TCP, скрываем в микшере
                reaper.SetMediaTrackInfo_Value(t, "B_SHOWINTCP", 1)
                reaper.SetMediaTrackInfo_Value(t, "B_SHOWINMIXER", 0)
            elseif mixer_mode == 'b' then
                -- Both режим: скрываем везде
                reaper.SetMediaTrackInfo_Value(t, "B_SHOWINTCP", 0)
                reaper.SetMediaTrackInfo_Value(t, "B_SHOWINMIXER", 0)
            end
        end
    ...
    -- Показываем саму папку и её содержимое
    local function ShowTrack(t)
        reaper.SetMediaTrackInfo_Value(t, "B_SHOWINTCP", 1)
        reaper.SetMediaTrackInfo_Value(t, "B_SHOWINMIXER", 1)
    end

    ShowTrack(folder_track)
    
    local j = folder_index + 1
    ...
    while j < track_count do
        ...
        if child_depth > 0 then
            ShowTrack(child_track)
        else
            break
        end
        
        current_level = current_level + child_folder_depth
        if current_level <= 0 then break end
        j = j + 1
    end
    
    -- Обновляем инфо панель
    local _, folder_name = reaper.GetSetMediaTrackInfo_String(folder_track, "P_NAME", "", false)
    tn = "FOLDER: " .. folder_name
    
    -- Считаем сколько треков сейчас видно
    local count = 0
    for i = 0, track_count - 1 do
        local t = reaper.GetTrack(0, i)
        if reaper.GetMediaTrackInfo_Value(t, "B_SHOWINTCP") == 1 then
            count = count + 1
        end
    end
    tfc = count
    ...
    reaper.TrackList_AdjustWindows(false)
    reaper.UpdateTimeline()
    reaper.Undo_EndBlock("Set Folder Visibility", -1)
end

function folder_btn(name, track, index, i)
    if r.ImGui_Button(ctx, name .. "##folder" .. i) then
        SetFolderVisibility(track, index, "t") -- Левый клик: TCP
    end

    -- Обработка правой и средней кнопки мыши
    if r.ImGui_IsItemHovered(ctx) then
        if r.ImGui_IsMouseClicked(ctx, 1) then
            SetFolderVisibility(track, index, "m") -- Правая кнопка: Mixer
        elseif r.ImGui_IsMouseClicked(ctx, 2) then
            SetFolderVisibility(track, index, "b") -- Средняя кнопка: Both
        end
    end
...
local function myWindow()
    ...
    r.ImGui_Separator(ctx)
    r.ImGui_Spacing(ctx)
    r.ImGui_TextColored(ctx, 0xFFFFFFBB, 'FOLDER GROUPS (Top-level folders):')
    r.ImGui_Spacing(ctx)

    -- Отрисовка второго ряда кнопок (Папки)
    for i, f_data in ipairs(folder_buttons) do
        folder_btn(f_data.name, f_data.track, f_data.index, i)
    end
end
