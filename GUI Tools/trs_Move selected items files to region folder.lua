-- @description Move selected items files to region folder (GUI)
...
-- @about
--   Moves files of selected items to a folder named after the region under the edit cursor.
--   Features a GUI to preview changes.

local ctx = reaper.ImGui_CreateContext('Move Files to Region Folder by Taras Umanskiy v.1.0')
local FONT_SIZE = 13
...
local function ScanItems()
    ...
    -- Get Region at Edit Cursor Position
    local cursor_pos = reaper.GetCursorPosition()
    local region_name = GetRegionAtTime(cursor_pos)
    ...
    -- Sanitize region name for folder usage
    local safe_region_name = region_name:gsub('[<>:"/\\|?*]', '_')
...
local function loop()
    ...
    local visible, open = reaper.ImGui_Begin(ctx, 'Move Files to Region Folder by Taras Umanskiy v.1.0', true, window_flags)
