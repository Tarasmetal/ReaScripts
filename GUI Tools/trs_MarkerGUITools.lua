...
if not widgets.cheads then
    ...
    if widgets.cheads.closable_group then
      ...
      -- if rv then
        ...
        r.ImGui_SameLine(ctx)
        btnCmdCol('Del M', '40613', 'Markers: Delete marker near cursor', 7,2)
        r.ImGui_SameLine(ctx)
        btnCmdCol('Del M all', '_SWSMARKERLIST9', 'Delete all Markers', 6,7)
        r.ImGui_SameLine(ctx)
        r.ImGui_TextDisabled(ctx, '|')
        r.ImGui_SameLine(ctx)
        btnCmdCol('Del R', '40615', 'Region: Delete marker near cursor', 6,0)
        r.ImGui_SameLine(ctx)
