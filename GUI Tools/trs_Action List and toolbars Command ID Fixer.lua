...
-- @changelog
--   + v2.1: Исправлена ошибка с символами "_-_" в новых ID (теперь заменяются на "_")
--   + v2.0: Изменен цвет текста в колонках Old ID (желтый) и New ID (зеленый) для лучшей читаемости
...
title = 'Action List and toolbars Command ID Fixer'
VERSION = '2.1'
author = 'Taras Umanskiy'
...
local size = r.GetAppVersion():match('Win64') and 12 or 13
  ...
  if is_analyzed and #analysis_results > 0 then
      ...
      if r.ImGui_BeginTable(ctx, 'preview_table', 4, table_flags, 0, 300) then
          ...
          for i, item in ipairs(analysis_results) do
              ...
              r.ImGui_TextColored(ctx, 0xD0D000FF, item.old_id)

              r.ImGui_TableSetColumnIndex(ctx, 3)
              r.ImGui_TextColored(ctx, 0x00AA00FF, item.new_id)
          end
