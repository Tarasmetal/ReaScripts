...
function loop()
  ...
  r.ImGui_BulletText(ctx, 'CTRL+Tab to select a window.')
     ...
     reaper.ImGui_Text(ctx, '')
      -- r.ImGui_SameLine(ctx)
      r.ImGui_TextColored(ctx, 0xFFFFFFFF, 'Test Velocity Range 120 + 10')
      r.ImGui_TextColored(ctx, 0xFFFF00FF, 'Vel:')
      r.ImGui_SameLine(ctx)
      r.ImGui_TextColored(ctx, 0xFF0036FF, rand(120,0))
      r.ImGui_TextColored(ctx, 0xFFFF00FF, 'Hum:')
      r.ImGui_SameLine(ctx)r.ImGui_TextColored(ctx, 0x88FF00FF, rand(120,10))
     r.ImGui_Text(ctx, '')
      r.ImGui_SameLine(ctx)
     r.ImGui_Text(ctx, '')
