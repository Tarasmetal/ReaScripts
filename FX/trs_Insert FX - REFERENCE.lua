    ...
    local NameFX = "Tonal Balance Control 2 (iZotope, Inc.)" --( = "" OFF / = "name" ON)
              -- = | "ВВЕДИТЕ СЮДА ИМЯ ПЛАГИНА"
              -- = | "ENTER HERE THE NAME OF THE PLUGIN"
              ------------------------------------------


    local NamePreset = "" --( = "" OFF / = "name" ON)
    ...
    local AdditionalFxName      = "ADPTR MetricAB (Plugin Alliance)"            --( = "" OFF / = "name" ON)
    local AdditionalNextFxName  = "REFERENCE (Mastering the Mix)"  --( = "" OFF / = "name" ON)
    local AdditionalNextNextFxName  = "Youlean Loudness Meter 2 (Youlean) (6ch)"  --( = "" OFF / = "name" ON)
    ...
    AdditionalNextFxName = AdditionalNextFxName:gsub(".+:%s+","");
    ...
    for j = 0,reaper.CountSelectedTracks(0) do
        ...
        ----
            ...
            --------/ AdditionalFx /--------
            if tostring(AdditionalNextFxName)then
                if #AdditionalNextFxName > 3 then
                    if IDX >= 0 then pos = 2 else pos = 1 end --(IDX ► Insert Fx)
                    local Idx = reaper.TrackFX_AddByName(SelTrack,AdditionalNextFxName,false,-1)
                    reaper.TrackFX_SetPreset(SelTrack,Idx,AdditionalNamePreset)
                    reaper.TrackFX_Show(SelTrack,Idx,0);--V.1.08
                    reaper.TrackFX_Show(SelTrack,Idx,2);--V.1.08
                    if Idx >= 0 then
