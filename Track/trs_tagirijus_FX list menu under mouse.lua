...
function OpenFXMenu()
	...
	-- TRACK = reaper.GetSelectedTrack(0, 0) -- old select
...
function modifyFXNameForEnabled(track, fxid, FXName)
	if not reaper.TrackFX_GetEnabled(track, fxid) then
		FXName = '' .. FXName .. ' • BYPASSED'
	elseif reaper.TrackFX_GetOffline(track, fxid) then
		FXName = '' .. FXName .. ' • OFFLINE'
	end
	return FXName
end

-- function modifyFXNameForOffline(track, fxid, FXName)
-- 	if not reaper.TrackFX_GetOffline(track, fxid) then
-- 		FXName = '' .. FXName .. ' • OFFLINE'
-- 	end
-- 	return FXName
-- end
...
function toggleOffline(track, fxid) -- Taras New func
	local offline_state = reaper.TrackFX_GetOffline(track, fxid)
	reaper.TrackFX_SetOffline(track, fxid, not offline_state)
end

	-- if not GetEnabled then;
 --                GetEnabled = reaper.TrackFX_GetOffline(TRACK,FXINDEX-1);
 ...
 --                Undo = true;
 --            end;
 --            reaper.TrackFX_SetOffline(TRACK,FXINDEX-1,SetEnabled);
function main()
	reaper.Undo_BeginBlock()
	...
	if FXINDEX > 0 then
		...
		if ctrl == 4 and shift == 0 then
			...
			-- toggleOffline(TRACK, FXINDEX - 1) -- New
			toggleOffline(TRACK, FXINDEX - 1) -- New
		...
		elseif ctrl == 4 and shift == 8 then
			...
			reaper.TrackFX_Delete(TRACK, FXINDEX - 1) -- old
		else
			-- no Ctrl or Shift is held down during click
			ShowOrHideOrFocus(TRACK, FXINDEX - 1)
		end
	end
	reaper.Undo_EndBlock("trs_Tagirijus: FX list menu", -1)
end
main()
