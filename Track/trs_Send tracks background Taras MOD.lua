...
--    + clear matrix button
  ...
  for key in pairs(reaper) do _G[key]=reaper[key]  end
       ...
       scr_title = "MPL Send tracks | Mod by T@RvZ"
       defsendvol = ({BR_Win32_GetPrivateProfileString( 'REAPER', 'defsendvol', '0',  reaper.get_ini_file() )})[2]
       defsendflag = ({BR_Win32_GetPrivateProfileString( 'REAPER', 'defsendflag', '0',  reaper.get_ini_file() )})[2]
  --init
  debug = 0
  --  INIT -------------------------------------------------
  for key in pairs(reaper) do _G[key]=reaper[key]  end
          ...
          for r in pairs(t[c]) do
            ...
            if conf.matrix == 0 then
              ...
              if s_id and d_id then
                ...
                -- SetTrackSendInfo_Value( src_tr, 0, new_id, 'I_SENDMODE', ({BR_Win32_GetPrivateProfileString( 'REAPER', 'defsendflag', '0',  reaper.get_ini_file() )})[2])
  ...
  ---------------------------------------------------------------------
    ...
    function Menu()
      local t = { {str = 'Show Audio Routing Matrix',
        ...
        --     {str = '|Change PreFX • PostFX • PostPan',
        --     state = false,
        --     func =  function()
        --     local ret, dest_val = reaper.GetUserInputs("preFx - 1 • postFx - 3 • postPan - 0", 1, "Select Send Fader: ", "")
        --   if not ret then return end

        --   if tonumber(dest_val) then
            -- defsendflag = tonumber(dest_val)
         --   end
        --     if not defsendflag then return end
        --     end},
        {str = '|• Pre-FX •',
        state = false,
        func =  function()
                scr_title = "MPL Send tracks • Pre-FX •"
                defsendflag = 1
                ExtState_Save()
        end},
         {str = '• Pre-Fader (Post-FX) •',
         state = false,
        func =  function()
                scr_title = "MPL Send tracks • Pre-Fader (Post-FX) •"
                defsendflag = 3
                ExtState_Save()
        end},
        {str = '• Pre-Fader (Post-Pan) •',
         state = false,
        func =  function()
                scr_title = "MPL Send tracks • Pre-Fader (Post-Pan) •"
                defsendflag = 0
                ExtState_Save()
        end},
        {str = '|• Post-FX (VI deprecated) •',
         state = false,
        func =  function()
                scr_title = "MPL Send tracks • Post-FX (VI) •"
                defsendflag = 2
                ExtState_Save()
        end},
         {str = '• Send Empty •',
         state = false,
        func =  function()
                scr_title = "MPL Send tracks • Send Empty •"
                defsendflag = -1
                ExtState_Save()
        end},
          }
