...
- **PRESETS**: Открывает выпадающий список доступных пресетов (.txt).
  - **New**: Создать новый пустой пресет.
  - **Clear**: Очистить текущий пресет от всех кнопок.
  - **Delete**: Удалить файл текущего пресета.
...
- **LR**: Ищет пары треков с одинаковыми именами и добавляет к первому "L", а ко второму "R" (поддерживает разделители пробел, подчеркивание или дефис).
- **Clone**:
  - **Левый клик**: Добавляет имена выбранных треков в список пресетов.
  - **Правый клик**: Клонирует имена и пытается разделить их на префикс и суффиксы.
...
- **Кнопка START**: Большая мигающая кнопка в центре для пустых пресетов, позволяющая быстро добавить первую строку.
...
- **Add Suffix**: Добавить новую ячейку суффикса в текущую строку.
- **Add Row**: Вставить новую строку после текущей.
- **Delete Row**: Удалить всю строку целиком.
...
- **`Insert`**:
  - Наведение на основную кнопку — добавить новую строку.
  - Наведение на кнопку суффикса — добавить новый суффикс в строку.
- **`Delete`**:
  - Наведение на основную кнопку — удалить всю строку.
  - Наведение на кнопку суффикса — очистить текст суффикса.
